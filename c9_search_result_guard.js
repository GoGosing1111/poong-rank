(function () {
  ...
  function fallbackDict() {
    return {
      "팬덤비하": [
        {"word":"길견","score":3},{"word":"똥퀴","score":3},{"word":"염퀴","score":3},{"word":"물소","score":2}
      ],
      "타퀴의심": [
        {"word":"민심","score":1},{"word":"박살","score":2},{"word":"손절","score":2}
      ],
      "이간질": [
        {"word":"갈라치기","score":2},{"word":"분열","score":2},{"word":"내분","score":2}
      ]
    };
  }
  ...
  function scanTitle(title, keywords) {
    ...
    keywords.forEach(function (k) {
      ...
      if (count) hits.push({category:k.category, word:k.word, score:k.score, count:count, sum:k.score * count});
    });
    return hits;
  }
  ...
  function render(rows) {
    ...
    p.innerHTML =
      ...
      '<div style="margin-top:4px;color:#ffb4b4;font-size:13px;font-weight:900;">' + esc(judge(total)) + ' · 점수 ' + total + '점 · 감지글 ' + rows.length + '건</div>' +
  ...
  loadKeywords().then(function (keywords) {
    ...
    document.querySelectorAll('iframe.yg-frame').forEach(function (f) {
      f.addEventListener('load', function () {
        setTimeout(function () { run(keywords); }, 700);
      });
    });
    setTimeout(function () { run(keywords); }, 1800);
    setTimeout(function () { run(keywords); }, 3500);
  });
})();
