...
def render_nick_search_card():
    """와이고수 3개 게시판 닉네임 통합검색 카드.
    - 검색 결과 새 탭 안에 인터넷방송/스타방송/스타대학 iframe 3개 출력
    - 새 탭 내부에서 bad_words.json을 불러와 각 게시판 검색결과 제목을 스캔
    - 감지 결과가 있으면 새 탭 상단 팝업으로 표시
    """
    srcdoc = """<!doctype html>
<meta charset='utf-8'>
<style>
...
.hint{margin-top:9px;color:#cbd5e1;font-size:11px;font-weight:800;line-height:1.45;text-align:center}.hint b{color:#7ec8ff}
.err{margin-top:6px;color:#ff8a8a;font-size:11px;font-weight:900;text-align:center;display:none}
</style>
<div class='wrap'>
  <div class='row'>
    <input id='q' class='inp' placeholder='닉네임 입력 예) 족버지' autocomplete='off'>
    <button id='go' class='btn' type='button'>검색</button>
  </div>
  <div id='err' class='err'>닉네임을 입력하세요.</div>
  <div class='hint'><b>SOOP(숲) · 인터넷방송 · 스타방송 · 스타대학</b><br>새 탭 한 페이지에 4개 검색결과 + 제목 감지 팝업을 띄웁니다.</div>
</div>
<script>
(function(){
  var BAD_WORDS_URL='https://keyman1335-maker.github.io/poong-rank/bad_words.json?v=202606130302';
  var boards=[
  ...
  function safeText(s){return String(s||'').replace(/[&<>\"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]||c})}
  function search(){
    var q=document.getElementById('q').value.trim();
    var err=document.getElementById('err');
    if(!q){err.style.display='block';return}
    err.style.display='none';
    var enc=encodeURIComponent(q);
    var frames=boards.map(function(b,idx){
      ...
      return '<div style="flex:1;min-width:360px;height:100%;display:flex;flex-direction:column;border-right:1px solid #334155;background:#111827;">'
        +'<div style="height:42px;display:flex;align-items:center;justify-content:center;background:#0f172a;color:#fff;font-weight:900;font-size:14px;border-bottom:1px solid #334155;">'+safeText(b.name)+' 검색 : '+safeText(q)+'</div>'
        +'<iframe class="yg-frame" data-board="'+safeText(b.name)+'" src="'+src+'" style="flex:1;width:100%;border:0;background:#fff;"></iframe>'
        +'</div>';
    }).join('');
    var w=window.open('about:blank','_blank');
    if(!w){alert('팝업이 차단되었습니다. 팝업 허용 후 다시 검색하세요.');return}
    w.document.open();
    w.document.write('<!doctype html><html><head><meta charset="utf-8"><title>와이고수 닉네임 검색 - '+safeText(q)+'</title></head><body style="margin:0;width:100vw;height:100vh;overflow:hidden;background:#111827;font-family:Arial,Malgun Gothic,sans-serif;"><div id="guard-pop" style="display:block;position:fixed;z-index:999999;left:50%;top:14px;transform:translateX(-50%);width:min(920px,calc(100vw - 24px));max-height:58vh;overflow:auto;border:1px solid #3b82f6;border-radius:16px;background:linear-gradient(135deg,#0f172a,#111827);box-shadow:0 18px 60px rgba(0,0,0,.55);color:#fff;"><div style="padding:10px 42px 10px 13px;background:rgba(15,23,42,.94);border-bottom:1px solid rgba(59,130,246,.45);"><button id="guard-close-ready" style="position:absolute;right:10px;top:8px;width:28px;height:28px;border:0;border-radius:8px;background:#475569;color:#fff;font-weight:1000;cursor:pointer;">×</button><div style="font-size:16px;font-weight:1000;color:#fff;">🚨 게시글 제목 감지</div><div id="guard-status" style="margin-top:4px;color:#93c5fd;font-size:13px;font-weight:900;">제목 감지 준비중... · iframe 로딩 대기</div></div></div><div style="display:flex;width:100%;height:100%;overflow:auto;">'+frames+'</div><script>(function(){var BAD_WORDS_URL="'+BAD_WORDS_URL+'";var KEYWORDS=null;try{var rb=document.getElementById("guard-close-ready");if(rb){rb.onclick=function(){document.getElementById("guard-pop").style.display="none"}}}catch(_e){}window.onerror=function(msg){try{var st=document.getElementById("guard-status");if(st){st.style.color="#fca5a5";st.textContent="제목 감지 스크립트 오류 · "+String(msg).slice(0,90)}}catch(e){}};function setReady(msg){try{var st=document.getElementById("guard-status");if(st){st.textContent=msg}}catch(e){}}function esc(s){return String(s||"").replace(/[&<>\"]/g,function(c){return {"&":"&amp;","<":"&lt;",">":"&gt;","\\\"":"&quot;"}[c]||c})}function flat(data){var out=[];if(!data)return out;if(Array.isArray(data)){data.forEach(function(x){if(typeof x==="string")out.push({category:"감지",word:x,score:1});else if(x&&x.word)out.push({category:x.category||"감지",word:x.word,score:x.score||1})});return out}Object.keys(data).forEach(function(cat){var arr=data[cat];if(Array.isArray(arr)){arr.forEach(function(x){if(typeof x==="string")out.push({category:cat,word:x,score:1});else if(x&&x.word)out.push({category:cat,word:x.word,score:x.score||1})})}});return out}function fallback(){return {"팬덤비하":[{"word":"길견","score":3},{"word":"똥퀴","score":3},{"word":"염퀴","score":3},{"word":"물소","score":2}],"타퀴의심":[{"word":"민심","score":1},{"word":"박살","score":2},{"word":"손절","score":2}],"이간질":[{"word":"갈라치기","score":2},{"word":"분열","score":2},{"word":"내분","score":2}]}}function getKeywords(){return fetch(BAD_WORDS_URL,{cache:"no-store"}).then(function(r){return r.json()}).catch(function(){return fallback()}).then(function(j){KEYWORDS=flat(j);return KEYWORDS})}function collectTitles(doc){var titles=[];var sel="td.subject a, .subject a, .list_subject a, a[href*=\\\"/board/\\\"]";doc.querySelectorAll(sel).forEach(function(a){var t=(a.textContent||"").replace(/\\s+/g," ").trim();var href=a.href||"";if(t.length<2||t.length>120)return;if(/^(전체|인기|와토|공지|정보|사진|영상|이벤트|관리자|추천|댓글|목록)$/.test(t))return;if(!/\/board\//.test(href)&&!a.closest("td.subject,.subject,.list_subject"))return;titles.push({title:t,href:href})});var seen={};return titles.filter(function(x){var k=x.title+"|"+x.href;if(seen[k])return false;seen[k]=1;return true}).slice(0,80)}function scanText(text){var hits=[];var low=String(text||"").toLowerCase();(KEYWORDS||[]).forEach(function(k){var w=String(k.word||"").trim();if(!w)return;if(low.indexOf(w.toLowerCase())>=0)hits.push(k)});return hits}function judge(total){if(total>=6)return "🚨 팬덤비하자 / 타퀴의심 강함";if(total>=3)return "⚠️ 타퀴의심 / 이간질 의심";if(total>0)return "주의 키워드 감지";return "감지 없음"}function render(all){var totalScore=0;all.forEach(function(r){r.hits.forEach(function(h){totalScore+=Number(h.score||1)})});if(!all.length){var pop=document.getElementById("guard-pop");pop.innerHTML="<div style=\"padding:10px 42px 10px 13px;background:rgba(15,23,42,.94);border-bottom:1px solid rgba(59,130,246,.45);\"><button id=\"guard-close\" style=\"position:absolute;right:10px;top:8px;width:28px;height:28px;border:0;border-radius:8px;background:#475569;color:#fff;font-weight:1000;cursor:pointer;\">×</button><div style=\"font-size:16px;font-weight:1000;color:#fff;\">✅ 게시글 제목 감지</div><div style=\"margin-top:4px;color:#93c5fd;font-size:13px;font-weight:900;\">감지 없음 · 점수 0점 · 감지글 0건</div></div>";pop.style.display="block";pop.style.border="1px solid #3b82f6";document.getElementById("guard-close").onclick=function(){pop.style.display="none"};return}var byCat={};all.forEach(function(r){r.hits.forEach(function(h){var c=h.category||"감지";if(!byCat[c])byCat[c]=[];byCat[c].push(h.word)})});var catHtml=Object.keys(byCat).map(function(c){var uniq=[].filter.call(byCat[c],function(v,i,a){return a.indexOf(v)===i});return "<span style=\\\"display:inline-block;margin:3px;padding:5px 8px;border-radius:999px;background:#7f1d1d;border:1px solid #ef4444;color:#fff;font-size:12px;font-weight:900;\\\">"+esc(c)+" : "+esc(uniq.join(", "))+"</span>"}).join("");var list=all.slice(0,12).map(function(r){return "<div style=\\\"margin-top:8px;padding:9px;border-radius:10px;background:rgba(0,0,0,.25);border:1px solid rgba(255,255,255,.10);\\\"><div style=\\\"color:#7ec8ff;font-size:12px;font-weight:1000;\\\">"+esc(r.board)+"</div><div style=\\\"margin-top:3px;color:#fff;font-size:13px;font-weight:900;line-height:1.35;\\\">"+esc(r.title)+"</div><div style=\\\"margin-top:5px;color:#ffb4b4;font-size:12px;font-weight:900;\\\">감지: "+esc(r.hits.map(function(h){return h.word}).join(", "))+"</div></div>"}).join("");var pop=document.getElementById("guard-pop");pop.innerHTML="<div style=\\\"position:sticky;top:0;padding:12px 42px 10px 13px;background:rgba(15,23,42,.94);border-bottom:1px solid rgba(239,68,68,.45);\\\"><button id=\\\"guard-close\\\" style=\\\"position:absolute;right:10px;top:8px;width:28px;height:28px;border:0;border-radius:8px;background:#475569;color:#fff;font-weight:1000;cursor:pointer;\\\">×</button><div style=\\\"font-size:17px;font-weight:1000;color:#fff;\\\">🚨 게시글 제목 감지</div><div style=\\\"margin-top:4px;color:#ffb4b4;font-size:13px;font-weight:900;\\\">"+esc(judge(totalScore))+" · 점수 "+totalScore+"점 · 감지글 "+all.length+"건</div><div style=\\\"margin-top:7px;\\\">"+catHtml+"</div></div><div style=\\\"padding:10px 12px 13px;\\\">"+list+"</div>";pop.style.display="block";document.getElementById("guard-close").onclick=function(){pop.style.display="none"}}function scanFrame(frame){try{var doc=frame.contentDocument||frame.contentWindow.document;var board=frame.getAttribute("data-board")||"게시판";var rows=[];collectTitles(doc).forEach(function(t){var hits=scanText(t.title);if(hits.length)rows.push({board:board,title:t.title,href:t.href,hits:hits})});return rows}catch(e){return []}}function runScan(){if(!KEYWORDS)return;var all=[];document.querySelectorAll("iframe.yg-frame").forEach(function(f){all=all.concat(scanFrame(f))});render(all)}getKeywords().then(function(){setReady("키워드 로드 완료 · 검색결과 로딩중...");document.querySelectorAll("iframe.yg-frame").forEach(function(f){f.addEventListener("load",function(){setTimeout(runScan,700)})});setTimeout(runScan,1800);setTimeout(runScan,3500)})})();<\/script></body></html>');
    w.document.close();
  }
  ...
  document.getElementById('q').addEventListener('keydown',function(e){if(e.key==='Enter')search()});
    ...
    iframe_html = f"""<iframe height="132" frameborder="0" allow="popups; clipboard-write" referrerpolicy="strict-origin-when-cross-origin" style="width:100%;border:0;border-radius:11px;overflow:hidden;background:#071426;" srcdoc="{_srcdoc_escape(srcdoc)}" src=""></iframe>"""
...
(function(){
  ...
  style="position:fixed;left:0;top:0;width:100vw;height:100vh;border:0;margin:0;border-radius:0;overflow:hidden;background:transparent;z-index:2147483647;display:block;"
