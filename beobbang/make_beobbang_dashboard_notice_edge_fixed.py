...
SPECIAL_FILE = "beobbang_special.txt"
SPECIAL_TEMPLATE = "beobbang_special_sample.txt"
...
def ensure_special_file():
    """beobbang_special.txt가 없으면 샘플 파일을 만들어 둠.
    사용자가 이 파일만 수정하면 다음 실행 때 오늘의 특이사항에 바로 반영됨.
    """
    if not os.path.exists(SPECIAL_FILE):
        sample = "오늘은 따끈한 특이사항이 없습니다.\n"
        try:
            open(SPECIAL_FILE, "w", encoding="utf-8").write(sample)
            print(f"[CREATE] {SPECIAL_FILE} 생성")
        except Exception as e:
            print(f"[WARN] {SPECIAL_FILE} 생성 실패: {e}")


def extract_special(text):
    ensure_special_file()
    # 1순위: beobbang_special.txt 직접 연동
    local = load_local_text(SPECIAL_FILE)
    if local:
        return local

    # 2순위: 구버전 파일명 호환
    local = load_local_text("dashboard_notice.txt", "notice.txt")
    if local:
        return local

    # 3순위: 공지 원문에서 키워드 기반 자동 추출
...
def section(title, body, open_attr=False, color=ORANGE, icon="🍞"):
    """SOOP 호환용 섹션.
    details/summary는 SOOP 에디터에서 테두리/모서리가 깨지는 경우가 있어 사용하지 않음.
    둥근 엣지는 유지하되 overflow:hidden 없이 상단/하단을 각각 둥글게 처리.
    """
    return f"""
<div style="margin:12px 0 14px;padding:0;border:2px solid {color};border-radius:22px;background:{PAPER};box-sizing:border-box;">
  <div style="padding:13px 14px;background:#ffe0b9;color:{BROWN};font-size:18px;font-weight:1000;text-align:center;border-bottom:2px dashed {color};letter-spacing:-.3px;border-radius:20px 20px 0 0;box-sizing:border-box;">
    {icon} {esc(title)}
  </div>
  <div style="padding:12px;background:#fff7ec;box-sizing:border-box;border-radius:0 0 20px 20px;">
    {body}
  </div>
</div>"""
...
def metric(label, value, icon="•"):
    return f"""<div style="display:inline-block;vertical-align:top;width:31.5%;min-width:150px;margin:4px .5%;padding:11px 8px;border-radius:16px;background:#fffdf8;border:2px solid #f3c58c;box-sizing:border-box;text-align:center;">
...
def member_card(m, idx=0):
    ...
    return f"""
<div style="display:inline-block;vertical-align:top;width:{width};margin:0 .6% 9px;padding:12px 8px 13px;border-radius:19px;background:{card_bg};border:2px solid #f3c58c;text-align:center;box-sizing:border-box;">
  ...
  {img_html}
  <div style="margin-top:8px;color:#4a220f;font-size:20px;font-weight:1000;white-space:nowrap;text-overflow:ellipsis;letter-spacing:-.5px;">{esc(m.get('name','-'))}</div>
  ...
  <a href="{esc(m.get('soop_url') or '#')}" target="_blank" rel="nofollow" style="display:inline-block;margin-top:8px;padding:6px 10px;border-radius:999px;background:#f97316;border:1px solid #c2410c;color:#fff;font-size:11px;font-weight:1000;text-decoration:none;">방송국 바로가기</a>
</div>"""
...
def render_schedule(items):
    ...
    for x in items[:10]:
        ...
        blocks.append(f'''
<div style="margin-bottom:8px;padding:11px 12px;border-radius:16px;background:#fffaf0;border:2px solid #fed7aa;color:#5b2a12;box-sizing:border-box;">
  <div style="display:inline-block;padding:4px 9px;border-radius:4px;background:#f97316;color:#fff;font-size:11px;font-weight:1000;">{esc(date or '일정')}</div>
...
def render_notice(post):
    ...
    # 마지막 공지사항 섹션은 바깥 section()이 이미 둥근 테두리를 가지고 있음.
    # SOOP 에디터에서 안쪽 박스까지 둥근 테두리/배경을 겹치면 하단 엣지가 깨지는 경우가 있어
    # 안쪽 공지 카드는 별도 외곽 둥근 테두리를 제거하고, 내용 박스만 낮은 radius로 처리.
    return f"""
<div style="margin:0;padding:0;background:#fff7ec;box-sizing:border-box;">
  <div style="padding:10px 11px;margin:0 0 8px;background:#fff3df;border:2px solid #f3c58c;border-radius:14px;box-sizing:border-box;">
    <div style="color:#5b2a12;font-size:16px;line-height:1.35;font-weight:1000;letter-spacing:-.4px;word-break:keep-all;">🍞 {esc(post.get('title','버빵동 공지사항'))}</div>
  </div>
  <div style="padding:11px 12px;background:#fffdf8;border:1px solid #fed7aa;border-radius:14px;color:#5b2a12;font-size:13px;line-height:1.62;font-weight:800;word-break:break-word;box-sizing:border-box;">{esc(body)}</div>
  <div style="padding-top:10px;text-align:left;">
    <a href="{esc(post.get('url') or POST_URL)}" target="_blank" rel="nofollow" style="display:inline-block;padding:8px 12px;border-radius:14px;background:#7c2d12;color:#fff;font-size:12px;font-weight:1000;text-decoration:none;">원문 확인</a>
  </div>
</div>"""
...
def render_hero(updated):
    return f"""
<div style="position:relative;border-radius:24px;border:2px solid #f3c58c;background:#ffe7c2;box-sizing:border-box;">
...
def main():
    ...
    html_out = f"""<div style="width:100%;max-width:760px;margin:0 auto;background:#fff1dc;padding:10px;box-sizing:border-box;font-family:Arial,'Malgun Gothic',sans-serif;color:#5b2a12;border-radius:24px;border:2px solid #f3c58c;">
  {render_hero(updated)}
  ...
  <div style="margin-top:12px;text-align:center;color:#8a4a21;font-size:11px;font-weight:900;">{esc(WATERMARK)}</div>
