...
def render_nick_search_card():
    """와이고수 3개 게시판 닉네임 검색센터.
    - 와고 본문에서도 동작하도록 iframe srcdoc 내부에 JS를 넣는다.
    - 닉네임 입력 후 검색 시 새 탭으로 인터넷방송/스타방송/스타대학 검색 결과를 연다.
    """
    iframe_html = """<iframe height="116" frameborder="0" referrerpolicy="strict-origin-when-cross-origin" style="display:block;width:100%;border:0;border-radius:11px;overflow:hidden;background:#071426;" srcdoc="&lt;!doctype html&gt;
&lt;meta charset='utf-8'&gt;

&lt;style&gt;
*{box-sizing:border-box}
body{margin:0;background:#071426;color:#fff;font-family:Arial,'Malgun Gothic',sans-serif;overflow:hidden}
.wrap{padding:10px;background:linear-gradient(135deg,rgba(47,155,255,.16),rgba(0,0,0,.25));border:1px solid rgba(126,200,255,.28);border-radius:11px}
.row{display:flex;gap:8px;width:100%}
input{flex:1;min-width:0;height:42px;border:1px solid rgba(126,200,255,.45);border-radius:9px;background:#020617;color:#fff;padding:0 12px;font-size:14px;font-weight:900;outline:none}
input::placeholder{color:#7f9bb7}
button{width:92px;height:42px;border:0;border-radius:9px;background:linear-gradient(135deg,#2f9bff,#635bff);color:#fff;font-size:14px;font-weight:1000;cursor:pointer}
.desc{margin-top:8px;text-align:center;color:#cbd5e1;font-size:11px;font-weight:800;line-height:1.35}
&lt;/style&gt;

&lt;div class='wrap'&gt;
  &lt;div class='row'&gt;
    &lt;input id='nick' type='text' placeholder='닉네임 입력' autocomplete='off'&gt;
    &lt;button id='btn' type='button'&gt;검색&lt;/button&gt;
  &lt;/div&gt;
  &lt;div class='desc'&gt;인터넷방송 · 스타방송 · 스타대학 검색 결과를 새 탭으로 엽니다.&lt;/div&gt;
&lt;/div&gt;
...
(function(){
  ...
  var btn=document.getElementById('btn');
  var boards=[
    'https://ygosu.com/board/ib?searcht=nick&amp;search=',
    'https://ygosu.com/board/starbbs?searcht=nick&amp;search=',
    'https://ygosu.com/board/pan_monstarz?searcht=nick&amp;search='
  ];
  function run(){
    var nick=(input.value||'').trim();
    if(!nick){ alert('닉네임을 입력하세요'); input.focus(); return; }
    var q=encodeURIComponent(nick);
    for(var i=0;i&lt;boards.length;i++){
      window.open(boards[i]+q, '_blank');
    }
  }
  btn.onclick=run;
...
" src=""></iframe>"""
    ...
    return f"""
<div id="section_nick_search" style="margin-top:10px;border:1px solid rgba(126,200,255,.46);border-radius:15px;background:linear-gradient(135deg,rgba(47,155,255,.16),rgba(0,0,0,.26));overflow:hidden;box-sizing:border-box;">
  <div style="padding:10px 12px;background:rgba(0,0,0,.24);border-bottom:1px solid rgba(126,200,255,.26);color:#7ec8ff;font-size:14px;font-weight:1000;text-align:left;text-shadow:0 2px 0 #000;">
    🔎 닉네임 검색센터
  </div>
  ...
  <div style="padding:12px 11px;color:#fff;font-size:13px;font-weight:900;line-height:1.62;text-align:center;word-break:keep-all;box-sizing:border-box;">
    와이고수 닉네임을 입력하면<br>
    <span style="color:#7ec8ff;font-weight:1000;">인터넷방송 · 스타방송 · 스타대학</span> 검색 결과를 새 탭으로 엽니다.
    ...
    <div style="margin-top:10px;color:#cbd5e1;font-size:11px;font-weight:800;line-height:1.45;">
      ※ 팝업 차단이 켜져 있으면 새 탭 3개 중 일부가 막힐 수 있습니다.
    </div>
  </div>
</div>"""
  ...
  <div class='popup-box'>
    ...
    <div class='popup-body'>
      <div class='grid'>
        ...
        <button class='item nav-item' data-target='section_nick_search' type='button'><div class='ico'>🔎</div><b>닉네임 검색</b><span>3개 게시판 검색 결과 새 탭 열기</span></button>
      </div>
    </div>
